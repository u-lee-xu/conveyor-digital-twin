...
import { SCORING_MODULES } from '../hooks/useRobotScoring';
...
export function RobotScoringPanel({ connected }: { connected: boolean }) {
  const {
    isScoringRunning,
    scoringComplete,
    setScoringRunning,
    ...
    <div className="space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-10">
      ...
      <div className="p-4 bg-indigo-500/5 rounded-xl border border-indigo-500/10 opacity-60">
        ...
        <p className="text-[9px] text-slate-500 leading-relaxed">
          纯累加制评分，初始 0 分，通过评分项累计至满分 100。前置子模块失败则后续子模块自动跳过，功能失败则跳过本模块剩余项。全自动测试流程，无需手动操作。缩回动作兼容单/双电控（双电控自保持 / 单电控失电复位）。
        </p>
...
export default RobotScoringPanel;
