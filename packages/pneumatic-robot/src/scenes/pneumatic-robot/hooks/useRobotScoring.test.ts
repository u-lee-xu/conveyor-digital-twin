import { describe, it, expect, vi } from 'vitest';

vi.mock('../../../services/plc-websocket', () => ({
  plcService: {
    connect: vi.fn(),
    disconnect: vi.fn(),
    readVars: vi.fn(),
    writeVar: vi.fn(),
    writeVars: vi.fn(),
    getStatus: vi.fn(),
    protocol: 'modbus',
    setOnDisconnected: vi.fn(),
  },
}));

import { SCORING_MODULES } from './useRobotScoring';

describe('SCORING_MODULES - 数据完整性验证', () => {

  describe('模块结构', () => {
    it('应该有4个评分模块', () => {
      expect(SCORING_MODULES).toHaveLength(4);
    });

    it('每个模块应有正确的ID格式', () => {
      SCORING_MODULES.forEach((m, i) => {
        expect(m.id).toBe(`M${i + 1}`);
      });
    });

    it('每个模块应有名称', () => {
      SCORING_MODULES.forEach(m => {
        expect(m.name).toBeTruthy();
        expect(typeof m.name).toBe('string');
      });
    });

    it('每个模块应至少有1个子模块', () => {
      SCORING_MODULES.forEach(m => {
        expect(m.subModules.length).toBeGreaterThanOrEqual(1);
      });
    });
  });

  describe('分数完整性', () => {
    it('4个模块总分应为100分', () => {
      const total = SCORING_MODULES.reduce((sum, m) => sum + m.maxPoints, 0);
      expect(total).toBe(100);
    });

    it('每个模块的 maxPoints 应等于其所有评分项的分数之和', () => {
      SCORING_MODULES.forEach(m => {
        const itemsTotal = m.subModules
          .flatMap(sm => sm.items)
          .reduce((sum, item) => sum + item.points, 0);
        expect(itemsTotal).toBe(m.maxPoints);
      });
    });

    it('每个模块的 maxPoints 应为25分', () => {
      SCORING_MODULES.forEach(m => {
        expect(m.maxPoints).toBe(25);
      });
    });

    it('所有评分项的分数都应为正整数', () => {
      SCORING_MODULES.forEach(m => {
        m.subModules.forEach(sm => {
          sm.items.forEach(item => {
            expect(item.points).toBeGreaterThan(0);
            expect(Number.isInteger(item.points)).toBe(true);
          });
        });
      });
    });
  });

  describe('评分项ID唯一性', () => {
    it('所有评分项的ID应该唯一', () => {
      const allIds = SCORING_MODULES.flatMap(m =>
        m.subModules.flatMap(sm => sm.items.map(item => item.id))
      );
      const uniqueIds = new Set(allIds);
      expect(uniqueIds.size).toBe(allIds.length);
    });

    it('所有子模块的ID应该唯一', () => {
      const allSubModuleIds = SCORING_MODULES.flatMap(m =>
        m.subModules.map(sm => sm.id)
      );
      const uniqueIds = new Set(allSubModuleIds);
      expect(uniqueIds.size).toBe(allSubModuleIds.length);
    });
  });

  describe('前置子模块（prerequisite）', () => {
    it('每个模块应有至少1个前置子模块', () => {
      SCORING_MODULES.forEach(m => {
        const prerequisites = m.subModules.filter(sm => sm.isPrerequisite);
        expect(prerequisites.length).toBeGreaterThanOrEqual(1);
      });
    });

    it('前置子模块应排在前面', () => {
      SCORING_MODULES.forEach(m => {
        const firstSubModule = m.subModules[0];
        expect(firstSubModule.isPrerequisite).toBe(true);
      });
    });
  });

  describe('评分项字段完整性', () => {
    it('每个评分项应有id、name、points、desc', () => {
      SCORING_MODULES.forEach(m => {
        m.subModules.forEach(sm => {
          sm.items.forEach(item => {
            expect(item.id).toBeTruthy();
            expect(item.name).toBeTruthy();
            expect(item.points).toBeGreaterThan(0);
            expect(item.desc).toBeTruthy();
          });
        });
      });
    });
  });

  describe('模块内容验证（对照6条控制要求）', () => {
    it('M1 原点状态测试应包含正确评分项（要求①）', () => {
      const m1 = SCORING_MODULES[0];
      expect(m1.id).toBe('M1');
      expect(m1.name).toContain('原点');

      const allItemIds = m1.subModules.flatMap(sm => sm.items.map(i => i.id));
      expect(allItemIds).toContain('m1_fwd_rear');
      expect(allItemIds).toContain('m1_lift_rear');
      expect(allItemIds).toContain('m1_clamp_open');
      expect(allItemIds).toContain('m1_origin_light');
      expect(allItemIds).toContain('m1_no_alarm');
    });

    it('M2 取料流程应包含正确评分项（要求②⑤）', () => {
      const m2 = SCORING_MODULES[1];
      expect(m2.id).toBe('M2');
      expect(m2.name).toContain('取料');

      const allItemIds = m2.subModules.flatMap(sm => sm.items.map(i => i.id));
      expect(allItemIds).toContain('m2_btn');
      expect(allItemIds).toContain('m2_lift_ext_ctrl');
      expect(allItemIds).toContain('m2_lift_front');
      expect(allItemIds).toContain('m2_clamp_ctrl');
      expect(allItemIds).toContain('m2_clamp_close');
      expect(allItemIds).toContain('m2_lift_ret_ctrl');
      expect(allItemIds).toContain('m2_lift_rear');
    });

    it('M2 状态灯验证应包含绿/黄灯（要求⑤）', () => {
      const m2 = SCORING_MODULES[1];
      const allItemIds = m2.subModules.flatMap(sm => sm.items.map(i => i.id));
      expect(allItemIds).toContain('m2_run_light');
      expect(allItemIds).toContain('m2_proc_light');
    });

    it('M3 搬运放料流程应包含正确评分项（要求③④）', () => {
      const m3 = SCORING_MODULES[2];
      expect(m3.id).toBe('M3');
      expect(m3.name).toContain('搬运');

      const allItemIds = m3.subModules.flatMap(sm => sm.items.map(i => i.id));
      expect(allItemIds).toContain('m3_fwd_ext_ctrl');
      expect(allItemIds).toContain('m3_fwd_front');
      expect(allItemIds).toContain('m3_lift_ext2_ctrl');
      expect(allItemIds).toContain('m3_clamp_open_ctrl');
      expect(allItemIds).toContain('m3_clamp_open');
      expect(allItemIds).toContain('m3_fwd_ret_ctrl');
      expect(allItemIds).toContain('m3_fwd_rear');
    });

    it('M4 停止/急停测试应包含正确评分项（要求⑤⑥）', () => {
      const m4 = SCORING_MODULES[3];
      expect(m4.id).toBe('M4');
      expect(m4.name).toContain('急停');

      const allItemIds = m4.subModules.flatMap(sm => sm.items.map(i => i.id));
      expect(allItemIds).toContain('m4_btn');
      expect(allItemIds).toContain('m4_run_off');
      expect(allItemIds).toContain('m4_estop_btn');
      expect(allItemIds).toContain('m4_alarm_on');
      expect(allItemIds).toContain('m4_restart_btn');
      expect(allItemIds).toContain('m4_alarm_off');
    });
  });

  describe('评分项总数', () => {
    it('应有合理数量的评分项（10-40项）', () => {
      const totalItems = SCORING_MODULES.flatMap(m =>
        m.subModules.flatMap(sm => sm.items)
      ).length;
      expect(totalItems).toBeGreaterThanOrEqual(10);
      expect(totalItems).toBeLessThanOrEqual(40);
    });
  });
});
